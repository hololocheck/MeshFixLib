...
- Safety mechanisms: triangle preservation check, pinch-point split, zero MC protection, volume sign check
...
└─ mesh-fix-core.wasm (~345KB, C++17 via Emscripten)
   ...
   └─ repairObject()      — 8-stage repair pipeline
      ...
      ├─ Stage 7: Hole Filling (pinch-point split + ear-clipping)
...
- 安全機構：三角形保存チェック、ピンチポイント分割、MC出力ゼロ保護、体積符号検証
...
└─ mesh-fix-core.wasm (~345KB, C++17 via Emscripten)
   ...
   └─ repairObject()      — 8段階修復パイプライン
      ...
      ├─ Stage 7: 穴埋め（ピンチポイント分割 + イヤークリッピング）
      └─ Stage 8: 最終クリーンアップ
      │
      ├─ エッジフリップWI修復
      ├─ 局所リメッシュWI修復
